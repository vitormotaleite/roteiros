...
public class RBTreeImpl<T extends Comparable<T>> extends BSTImpl<T>
	...
	private boolean verifyChildrenOfRedNodes() {
		if (verifyChildrenOfRedNodes2((RBNode<T>) root))
			return true;
		else
			return false;
	}
	
	private boolean verifyChildrenOfRedNodes2(RBNode<T> node) {
		...
		if (!node.isEmpty()) {
			...
			return verifyChildrenOfRedNodes2((RBNode<T>) node.getLeft())
					&& verifyChildrenOfRedNodes2((RBNode<T>) node.getRight());
		}
		return true;
	}
	...
	private boolean verifyBlackHeight() {
		if (verifyBlackHeight2((RBNode<T>) root))
			return true;
		else
			return false;
	}
	
	private boolean verifyBlackHeight2(RBNode<T> node) {
		...
		if (node.isEmpty()) {
			...
			result = (verifyBlackHeight2((RBNode<T>) node.getLeft()))
					&& (verifyBlackHeight2((RBNode<T>) node.getRight()));
	...
	public void insert(T value) {
		insert2((RBNode<T>) root, value);
	}
	
	private void insert2(RBNode<T> node, T element) {
		...
		if (node.isEmpty()) {
			...
			if (element.compareTo(node.getData()) > 0)
				insert2((RBNode<T>) node.getRight(), element);
			else if (element.compareTo(node.getData()) < 0)
				insert2((RBNode<T>) node.getLeft(), element);
		}
	}
	...
	public RBNode<T>[] rbPreOrder() {
		...
		rbPreOrder2(arrayAux, (RBNode<T>) this.root);
	...
	private void rbPreOrder2(List<RBNode<T>> arrayAux, RBNode<T> node) {

		if (!node.isEmpty()) {
			arrayAux.add(node);
			rbPreOrder2(arrayAux, (RBNode<T>) node.getLeft());
			rbPreOrder2(arrayAux, (RBNode<T>) node.getRight());
		}
	}
